...
function SPAPI_Lists(baseUrl) {
    ...
    this.quickUpdateListItem = function(listName, fields, rootFolder) {
	    return this.callUpdateListItems(listName, fields, "Update", rootFolder);
	}
	...
	this.callFolderUpdate = function(listName, folderName, command, rootFolder, ItemID) {
	    var batch;
        ...
        batch += "<Method ID='1' Cmd='" + command + "'><Field Name='FSObjType'>1</Field>";
        
		if (command=="Update") {
	        batch += "<Field Name='ID'>" + ItemID + "</Field>";
	        batch += "<Field Name='owshiddenversion'>1</Field>";
	        batch += "<Field Name='FileRef'>" + rootFolder.substring(1,rootFolder.length) + "/" + folderName + "</Field>";
		}

        batch += "<Field Name='BaseName'>" + folderName + "</Field>"
	...
	this.updateFolder = function(listName, folderName, rootFolder, ItemID) {
	    return this.callFolderUpdate(listName, folderName, "Update", rootFolder, ItemID);
	}
}